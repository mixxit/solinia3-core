...
public class Solinia3CorePlugin extends JavaPlugin implements PluginMessageListener {
	...
	private SoliniaLivingEntityUpdateTargetWindowTimer soliniaLivingEntityUpdateTargetWindowTimer;
	...
	public void onEnable() {
		...
		if (expectedClientModVersion == null || expectedClientModVersion.equals("")) {
			if (!IsOffline()) {
				System.out.println(
						"Solinia3-Core Error loading plugin!!! Could not find expected mod version! Disabling plugin...");
				Bukkit.getPluginManager().disablePlugin(this);
				return;
			} else {
		...
		getServer().getMessenger().registerOutgoingPluginChannel(this, Solinia3UIChannelNames.Outgoing); // we register
																											// the
																											// outgoing
																											// channel
	...
	private void startProtocolListeners() {
		...
		protocolManager.addPacketListener(new ParticlePacketAdapter(
				PacketAdapter.params(this, new PacketType[] { PacketType.Play.Server.ANIMATION, PacketType.Play.Server.ENTITY_EFFECT, PacketType.Play.Server.ENTITY_METADATA, PacketType.Play.Server.ENTITY_STATUS })));
	}
	...
	private void loadDynmap() {
		...
		soliniaZonesSet = dynmapApi.getMarkerAPI().createMarkerSet("solinia.markerset", "SoliniaZones",
				dynmapApi.getMarkerAPI().getMarkerIcons(), false);
		soliniaZonesSet.setHideByDefault(true);
		regionExtentsSet = dynmapApi.getMarkerAPI().createMarkerSet("solinia.worldregionsmarkerset", "WorldRegions",
				dynmapApi.getMarkerAPI().getMarkerIcons(), false);
	...
	public void onPluginMessageReceived(String channel, org.bukkit.entity.Player player, byte[] bytes) {
		...
		/*
		 * ByteArrayDataInput in = ByteStreams.newDataInput(bytes); String subChannel =
		 * in.readUTF(); if (subChannel.equalsIgnoreCase("openspellbook")) { String
		 * data1 = in.readUTF(); int data2 = in.readInt();
		 * 
	...
	public void onDisable() {
		try {
			StateManager.getInstance().getEntityManager().removeAllPets(true);
	...
	private void initialise(ConfigSettings configSettings) {
		...
		try {
			...
			itemrepo.setJsonFile(getDataFolder() + "/" + "items");
			itemrepo.reload();
	...
	private void setupCommands() {
		...
		this.getCommand("unmentor").setExecutor(new CommandUnmentor());
		...
		this.getCommand("topplayers").setExecutor(new CommandTopPlayers());
		this.getCommand("scribeallspells").setExecutor(new CommandScribeAllSpells());
		this.getCommand("loadspells").setExecutor(new CommandLoadSpells(this));
		this.getCommand("savespells").setExecutor(new CommandSaveSpells(this));
		this.getCommand("togglepassive").setExecutor(new CommandTogglePassive());
		...
		this.getCommand("togglehint").setTabCompleter(new CommandToggleHint());
