	...
	public static void Patcher() {
		/*try
		{
			for (String entity : StateManager.getInstance().getConfigurationManager().getPatchesRepo().getByKey(1).getPlayerRestoreInventory().keySet()) {
				...
				for(ISoliniaPlayer player : StateManager.getInstance().getConfigurationManager().getCharactersByPlayerUUID(UUID.fromString(entity)).stream().limit(1).collect(Collectors.toList()))
				{
